...
| 1:02–1:22 | Open **Run → Verification**, show the detected project command suggestion, explicit automatic-verification switch, and saved command, then run it through the same command safety/approval boundary. If the first test fails, use the visible **Repair** action to show the bounded evidence-fed repair cycle; otherwise show the green deterministic receipt. Do not claim validation that is not visible. |
